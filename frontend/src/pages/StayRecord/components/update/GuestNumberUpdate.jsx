  ...
  const handleConfirmationAccept = async () => {
    ...
    try {
      const response = await axios.put(
        `${config.API_URL}/stay_records/${stayRecord.id}`,
        formValues
      );

      if (response.data.success) {
        onUpdate({ ...stayRecord, ...formValues });
        showSnackbar("Guest numbers updated successfully.", "success");
        onClose();
      } else {
        showSnackbar(response.data.message || "Failed to update guest numbers.", "error");
      }
    } catch (error) {
      console.error("Failed to update guest numbers:", error);
      showSnackbar("Server error occurred while updating guest numbers.", "error");
    }
  };
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {stayRecord && (
            <Formik
              initialValues={{
                adults: stayRecord.adults,
                kids: stayRecord.kids,
              }}
